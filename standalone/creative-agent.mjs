...
export function buildCreativeAgentSystemPrompt(context) {
  return [
    ...
    '先判断需求是否足够明确：如果用户首次提出创作需求，且缺少产品/主题、受众或平台、时长/规格、风格/参考中的任意关键项，不要直接交付完整脚本或方案；先用 text 简短说明需要补充信息，并在 questions 中提出 1 到 3 个具体问题。',
    '如果需求已经明确，questions 必须为空；只有在需要补充信息时才提问。',
    '必须只输出 JSON，不要输出 Markdown 代码围栏，格式为：',
    '{"text":"面向用户的自然语言回复","questions":["需要用户补充的具体问题"],"followUps":["最多三个可继续追问的问题"]}',
    'questions 是需求澄清问题，不是批准卡；followUps 只放已经回答完成后可继续追问的短问题。不要输出 cards、卡片、收藏或置信度字段，也不要声称已经修改了项目。',
