...
const els = Object.fromEntries([
  'projectSelect','newProjectBtn','saveState','addImageBtn','addVideoBtn','uploadBtn','exportBtn','fileInput',
...
function friendlyError(v){if(['Agnes video image references require PUBLIC_BASE_URL pointing to this Studio','当前图片仅保存在本机，Agnes 视频无法访问。请重新运行 Agnes 图片节点后再生成视频；本地上传图片需在“模型/API → Agnes AI”配置素材公网地址'].includes(v))return '该图片没有 Agnes 可访问的公网地址。请配置素材公网地址，或改用支持内联图片的模型。';if(/video queue is full/i.test(String(v||'')))return '视频服务当前排队已满，自动重试仍未成功，请稍后点击重试。图片、提示词和参数已保留。';return v;}
...
async function openProject(projectId){
  ...
  const promptMigrated=migratePromptNodes(),layoutMigrated=migrateGenerationNodeLayout(),referencesMigrated=migrateNodeReferences();
  ...
  normalizeTimeline(); renderProjectSelect(); renderAll(); resumeNodeJobs(); if(promptMigrated||layoutMigrated||referencesMigrated)scheduleSave(); setTimeout(fitCanvas,0); if(els.agentDrawer&&!els.agentDrawer.classList.contains('hidden-drawer'))loadAgent();
}
...
function seedWelcomeNodes(){
  const prompt='一辆银色未来跑车在雨夜东京街头高速驶过，电影级灯光，低机位跟拍。';
  const b={id:id(),type:'imageGen',position:{x:20,y:40},data:{modelKey:firstModelKey('image.generate'),prompt,status:'idle',progress:0}};
  const c={id:id(),type:'videoGen',position:{x:560,y:40},data:{modelKey:firstModelKey('video.generate'),forcedCapability:'video.generate',prompt,status:'idle',progress:0,params:{duration:5,aspectRatio:'16:9',resolution:'720p'}}};
  S.workflow.nodes.push(b,c); scheduleSave();
}
...
function videoParamControls(n){const p=normalizedVideoParams(n),durationOptions=p.durations.map(v=>`<option value="${esc(v)}" ${String(v)===String(n.data.params.duration)?'selected':''}>${esc(v)}s</option>`).join(''),audioLabels={ambient:'环境音',silent:'静音',music:'音乐',voiceover:'口播',full:'完整'};return `<div class="inline-fields four"><div class="field"><label>时长</label><select data-param="duration" aria-label="时长">${durationOptions}</select></div><div class="field"><label>画幅</label><select data-param="aspectRatio" aria-label="画幅">${options(p.aspects,n.data.params.aspectRatio)}</select></div><div class="field"><label>分辨率</label><select data-param="resolution" aria-label="分辨率">${options(p.resolutions,n.data.params.resolution)}</select></div><div class="field"><label>声音</label><select data-param="audioMode" aria-label="声音">${p.audioModes.map(v=>`<option value="${esc(v)}" ${String(v)===String(n.data.params.audioMode)?'selected':''}>${esc(audioLabels[v]||v)}</option>`).join('')}</select></div></div>${timelineControl(n)}`;}
...
function createNodeElement(n){
    ...
    const extra=n.type==='imageGen'?'node-generation node-image':n.type==='videoGen'?'node-generation node-video':n.type==='textGen'?'node-generation node-textgen':n.type==='upload'?'node-generation node-upload':'';
...
const NODE_ACTIONS=[
  ...
  {id:'image.edit',nodeTypes:['imageGen'],label:'图片编辑',icon:'photo-edit',capability:'image.edit',requiredReferenceKinds:['image']},
...
const SCRIPT_TABLE_RULE=`必须按 Markdown 表格输出，固定表头只能是：${SCRIPT_TABLE_HEADER}。每行只写一个镜头；画面描述写清场景、主体动作、运镜、光线、构图和物理约束；文案/旁白、时长、音效/音乐必须分别放入对应列，没有内容时填写“—”。表格外不要重复镜头内容。`;
function textSystemForPreset(p){return ({storyboard:`把输入创意整理成可拍摄的分镜脚本。每个镜头写清景别、主体动作、镜头运动、环境、光线、时长与对白/音效。${SCRIPT_TABLE_RULE}`,video_script:`把输入主题写成结构清晰的视频脚本，包含开场钩子、内容推进、画外音、画面建议和结尾转化。${SCRIPT_TABLE_RULE}`,image_prompt:'把输入描述扩写为专业图像生成提示词，包含主体、环境、构图、镜头、光线、材质、色彩和风格，但不要改变核心意图。',nine_grid_hook:`你是 TikTok/抖音电商信息流短视频分镜专家。这个预设只生成“3x3 强 Hook 九宫格中文分镜脚本”，不要生成 Seedance 成片提示词，不要输出视频最终提示词，不要混入成片参数。${SCRIPT_TABLE_RULE}必须共9行；前3镜必须是强 Hook，有停滑点、痛点放大、反差或结果前置，并且第3镜前必须出现产品或解决方案；中间3镜展示核心卖点和使用场景；最后3镜完成信任背书和转化收束。表格前只保留一行统一视觉基调，表格后只询问用户是否确认。`,seedance_grid_prompt:`你是 Seedance 2.0/C端2.0 视频提示词工程师。这个预设只生成“九宫格成片 Seedance 最终视频提示词”，不要重新生成九宫格分镜脚本，也不要重新设计分镜或混入强 Hook 分镜内容。基于用户提供的九宫格脚本、9段图片提示词或上游文本，整理成可直接用于 Seedance 的最终视频生成提示词。${SCRIPT_TABLE_RULE}必须完整覆盖9镜，按从上到下、从左到右顺序填写具体时间码、景别/运镜、主体动作、卖点表达、场景、构图、光线、声音设计、产品一致性和真实物理约束。禁止字幕和画面文字，除非用户明确要求。`,rewrite:'保持原意，提升表达、结构和可执行性。'})[p]||'根据用户输入生成高质量文本。';}
...
function nodeActionEnabled(n,action){const kinds=new Set(rawNodeReferences(n).map(ref=>findAsset(ref.assetId)?.kind).filter(Boolean)),currentCapability=n.type==='videoGen'?videoNodeCapability(n):n.type==='imageGen'?imageNodeCapability(n):action.capability;if(action.capability&&action.capability!==currentCapability&&!modelsFor(action.capability).length)return false;return !(action.requiredReferenceKinds||[]).some(kind=>!kinds.has(kind))&&!(action.forbidReferenceKinds||[]).some(kind=>kinds.has(kind));}
...
function nodeReferenceChips(n){
  ...
  return `<div class="node-refs">${refs.map(r=>{
    ...
    const roleCtrl=(n.type==='videoGen'&&isPreset)?`<select data-ref-role="${esc(key)}" class="node-ref-role" aria-label="生成角色">${referenceRoleOptions(a,r.role)}</select>`:`<span class="node-ref-role-static">${esc(REFERENCE_ROLE_LABELS[r.role]||r.role)}</span>`;
    const semantic=isPreset?`<select data-ref-semantic="${esc(key)}" class="node-ref-role" aria-label="语义角色">${semanticRoleOptions(r.semanticRole)}</select>`:`<span class="node-ref-role-static">${esc(SEMANTIC_REFERENCE_ROLES[r.semanticRole]||r.semanticRole)}</span>`;
    const remove=isPreset?`<button data-ref-remove="${esc(key)}" title="移除参考" aria-label="移除参考">${icon('x')}</button>`:'';
    return `<div class="node-ref" data-asset="${esc(a.id)}">${thumb}<div class="node-ref-meta"><span class="node-ref-name">${esc(r.label||a.filename)}</span><span class="node-ref-source">${esc(referenceSourceLabel(r))}</span><div class="node-ref-roles">${roleCtrl}${semantic}</div>${referenceRegionEditor(r,isPreset)}</div>${remove}</div>`;
  }).join('')}</div>`;
}
...
function generationResultActions(n){
  ...
  const out=nodeOutputAssets(n).map(findAsset).find(a=>a&&['image','video'].includes(a.kind));
  if(!out)return '';
  const label=out.kind==='video'?'下载视频':'下载图片';
  return `<div class="generation-result-actions media-result-actions ${out.kind}-result-actions"><button class="media-download-action" data-action="downloadOutput" title="${label}" aria-label="${label}">${icon('download')}<span>${label}</span></button></div>`;
}
...
function parseStructuredScript(text,preset){return parseMarkdownTable(text)||(SCRIPT_PRESETS.has(preset)?parseLegacyScript(text):null);}
...
function refreshTextOutputTable(){
  ...
  els.textOutputTable.innerHTML=renderMarkdownTables(value,editing,preset);
  page?.classList.toggle('has-table',Boolean(parsed));
  if(els.textOutputMeta)els.textOutputMeta.textContent=parsed?`${parsed.rows.length} 个镜头${textOutputDuration(parsed)?` · 总时长约 ${textOutputDuration(parsed)} 秒`:''}`:'完整查看和修改文本生成结果';
}
...
function saveTextOutputEdits(){const value=textOutputDraftValue();els.textOutputEditor.value=value;syncTextOutputNode(value);S.textOutputOriginal=value;setTextOutputEditing(false);toast('脚本已保存');}
...
function pickReferenceAsset(nodeId,asset){
  ...
  scheduleSave();renderNode(n);renderInspector();
...
function nodeTemplate(n){
  ...
  if(n.type==='imageGen'){
    ...
    const preview=out?`<img class="node-media" src="${esc(out.publicUrl)}" title="双击放大预览">`:generationPlaceholder('photo-plus',[['upload','图生图'],['badge-hd','图片高清']]);
...
function bindNode(el,n){
  ...
  el.addEventListener('click',e=>{e.stopPropagation();if(S.interaction)return;const action=e.target.closest('[data-action]')?.dataset.action;if(action==='delete')return deleteNode(n.id);if(action==='chooseUpload')return chooseUploadFiles(n.id);if(action==='toggleComposer'){if(isGenerationNode(n)){n.data.expanded=!n.data.expanded;scheduleSave();updateComposerStateDom(el,n);}return;}if(action==='generate')return generateNode(n.id);if(action==='cancelGeneration')return cancelNodeGeneration(n);if(action==='openTextOutput')return openTextOutputPage(n);if(action==='copyOutput')return copyNodeOutput(n);if(action==='downloadOutput')return downloadNodeOutput(n);if(action==='timeline')return addAssetToTimeline(n.data.assetId);if(action==='timelineOutput')return addAssetToTimeline(nodeOutputAssets(n)[0]);if(action==='nodeAction')return applyNodeAction(n,e.target.closest('[data-node-action]').dataset.nodeAction);if(action==='open'){const a=findAsset(n.data.assetId);if(a)window.open(a.publicUrl,'_blank');return;}if(action==='refAdd'){S.assetPick={nodeId:n.id};showDrawer('assetDrawer');toast('点击素材库中的素材，添加为节点参考');return;}if(action==='refRemove'){const key=e.target.closest('[data-ref-remove]')?.dataset.refRemove;if(key)removeNodeReference(n,key);return;}if(action==='variant'){const aid=e.target.closest('[data-variant]')?.dataset.variant;if(aid)selectVariant(n,aid);return;}selectNode(n.id);});
  if(n.type==='upload'){
    ...
    $$('img.node-media, video.node-media',el).forEach(media=>media.addEventListener('dblclick',ev=>ev.stopImmediatePropagation()));
  }
  $$('.generator-text-result',el).forEach(textarea=>textarea.addEventListener('wheel',e=>e.stopPropagation(),{passive:true}));
  $$('video',el).forEach(media=>['pointerdown','click','dblclick'].forEach(type=>media.addEventListener(type,e=>e.stopPropagation())));
  $$('img.node-media, video.node-media',el).forEach(media=>media.addEventListener('dblclick',ev=>{ev.stopPropagation();const currentSrc=media.currentSrc||media.src,out=S.assets.find(asset=>new URL(asset.publicUrl,location.href).href===currentSrc)||nodeOutputAssets(n).map(findAsset).find(Boolean);if(out)openMediaLightbox(out);}));
  ...
  $$('[data-ref-semantic]',el).forEach(sel=>sel.addEventListener('change',()=>{beginCanvasSnapshot();updateNodeReference(n,sel.dataset.refSemantic,{semanticRole:sel.value});scheduleSave();renderNode(n);renderInspector();}));
...
function finishConnection(clientX,clientY){const it=S.interaction;if(!it||it.type!=='connect')return;const hit=document.elementFromPoint(clientX,clientY),targetNode=hit?.closest?.('.node'),targetId=targetNode?.dataset.id;S.interaction=null;els.connectionToast.classList.add('hidden');clearConnectionTargets();if(targetId&&targetId!==it.sourceId)createEdge(it.sourceId,targetId);else{S.canvasHistory.past.pop();updateUndoButtons();}renderEdges();}
...
function addUploadNode(position){return addNode('upload',{assetId:null,status:'idle',error:'',layoutWidth:GENERATION_NODE_W},position);}
...
function collectIncomingText(n){return S.workflow.edges.filter(e=>e.target===n.id).map(e=>nodeById(e.source)).filter(Boolean).map(nodeOutputText).filter(t=>t.trim()).join('\n\n').trim();}
...
function pendingReferenceKinds(n){
  ...
  for(const e of S.workflow.edges.filter(e=>e.target===n.id)){
    ...
    if(src.type==='asset'){const a=findAsset(src.data.assetId); if(a) kinds.add(a.kind);}
...
function edgeState(e){
  ...
  if(s.type==='asset') return findAsset(s.data.assetId)?'done':'pending';
...
async function generateNode(nodeId){
  ...
  const validation=n.type==='videoGen'?videoNodeValidationError(n):'';
  ...
  }catch(e){ n.data.status='failed'; n.data.error=e.message; renderNode(n); toast(e.message,'error'); return null; }
}
...
function renderAssets(){if(!S.assets.length){els.assetList.innerHTML=`<div class="empty-state asset-empty">${S.assetFilter?.tag!=='all'||S.assetFilter?.kind!=='all'?'当前过滤无结果，点击上方分类清除过滤':'暂无素材'}</div>`;return;}els.assetList.innerHTML=S.assets.map(a=>`<div class="asset-card" data-asset="${esc(a.id)}">${a.kind==='image'?`<img class="asset-thumb" src="${esc(a.publicUrl)}">`:a.kind==='video'?`<video class="asset-thumb" src="${esc(a.publicUrl)}" muted></video>`:`<div class="asset-thumb audio">${icon('volume')}</div>`}<div><div class="asset-name">${esc(a.filename)}</div><div class="asset-meta">${esc(a.kind)}${a.durationMs?` · ${(a.durationMs/1000).toFixed(1)}s`:''}</div>${a.tags?.length?`<div class="asset-tags">${a.tags.map(t=>`<span class="asset-tag">${esc(t)}</span>`).join('')}</div>`:''}<div class="asset-actions"><button data-a="canvas">${icon('layout-dashboard')}<span>Canvas</span></button><button data-a="timeline">${icon('timeline')}<span>Timeline</span></button><button data-a="open">${icon('external-link')}<span>打开</span></button></div></div></div>`).join('');$$('.asset-card',els.assetList).forEach(card=>card.addEventListener('click',e=>{const a=findAsset(card.dataset.asset);if(!a)return;if(S.assetPick){const pick=S.assetPick;S.assetPick=null;hideDrawer('assetDrawer');pickReferenceAsset(pick.nodeId,a);return;}const action=e.target.closest('[data-a]')?.dataset.a;if(action==='canvas')addAssetNode(a);else if(action==='timeline')addAssetToTimeline(a.id);else if(action==='open')window.open(a.publicUrl,'_blank');else showAssetInspector(a);}));}
function showAssetInspector(a){S.selectedNodeId=S.selectedEdgeId=S.selectedClipId=null;S.selectedNodeIds=[];updateCanvasSelectionDom();renderEdges();els.inspector.className='inspector';const tags=a.tags||[];els.inspector.innerHTML=`<h3>${esc(a.filename)}</h3><dl><dt>Type</dt><dd>${esc(a.kind)}</dd><dt>Asset ID</dt><dd>${esc(a.id)}</dd><dt>Size</dt><dd>${a.width||'-'} × ${a.height||'-'}</dd><dt>Duration</dt><dd>${a.durationMs?`${(a.durationMs/1000).toFixed(2)}s`:'-'}</dd><dt>URL</dt><dd>${esc(a.publicUrl)}</dd></dl><div class="section-divider"></div><div class="asset-tags-editor"><label class="field"><span>标签</span><div class="asset-tags">${tags.map(t=>`<span class="asset-tag">${esc(t)}<button data-tag-rm="${esc(t)}" title="移除标签">×</button></span>`).join('')}</div></label><div class="inline-fields two"><input id="newAssetTag" type="text" placeholder="添加标签后回车" list="assetTagSuggestions"><button id="addAssetTagBtn">添加</button></div><datalist id="assetTagSuggestions">${[...new Set(S.assets.flatMap(x=>x.tags||[]))].map(t=>`<option value="${esc(t)}">`).join('')}</datalist></div>`;$('#newAssetTag',els.inspector)?.addEventListener('keydown',ev=>{if(ev.key==='Enter'){ev.preventDefault();addAssetTag(a,$('#newAssetTag').value.trim());}});$('#addAssetTagBtn',els.inspector)?.addEventListener('click',()=>addAssetTag(a,$('#newAssetTag').value.trim()));$$('[data-tag-rm]',els.inspector).forEach(b=>b.addEventListener('click',()=>removeAssetTag(a,b.dataset.tagRm)));renderTimelineSelectionButtons();}
async function addAssetTag(a,tag){if(!tag)return;await api(`/api/projects/${S.projectId}/assets/${a.id}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({add:[tag]})});await refreshAssets();showAssetInspector(findAsset(a.id));}
async function removeAssetTag(a,tag){await api(`/api/projects/${S.projectId}/assets/${a.id}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({remove:[tag]})});await refreshAssets();showAssetInspector(findAsset(a.id));}
...
const NODE_CATALOG=[
  {id:'text.ai',icon:'sparkles',title:'AI 文本生成',create:(pos)=>addTextGen('rewrite',{},pos)},
  {id:'image.generate',icon:'photo-plus',title:'图片生成',create:(pos)=>addImage({},pos)},
  {id:'video.generate',icon:'video-plus',title:'文生视频',create:(pos)=>addVideo({forcedCapability:'video.generate'},pos)},
  {id:'asset.upload',icon:'upload',title:'上传节点',create:(pos)=>addUploadNode(pos)},
  {id:'asset.history',icon:'history',title:'从生成历史选择',create:()=>showDrawer('assetDrawer')},
];
...
function setupGlobalInteractions(){
  ...
  els.canvas.addEventListener('dragover',e=>{if(!hasDraggedFiles(e))return;e.preventDefault();els.canvas.classList.add('file-drag-active');});
  ...
  els.canvas.addEventListener('drop',e=>{if(!hasDraggedFiles(e))return;e.preventDefault();els.canvas.classList.remove('file-drag-active');const node=addUploadNode(screenToWorld(e.clientX,e.clientY));uploadFiles([...e.dataTransfer.files],{targetNodeId:node.id});});
...
els.textOutputEditBtn?.addEventListener('click',toggleTextOutputEditing);els.textOutputSaveBtn?.addEventListener('click',saveTextOutputEdits);
