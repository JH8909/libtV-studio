import { IMAGE_PRESET_CATEGORIES, imagePresetById, imagePresetsForCategory, closestSupportedAspectRatio, composePresetRequestPrompt, composeImagePresetPrompt } from './image-presets.js';

const $ = (s, root = document) => root.querySelector(s);
const $$ = (s, root = document) => [...root.querySelectorAll(s)];
const els = Object.fromEntries([
  'projectSelect','newProjectBtn','saveState','addImageBtn','addVideoBtn','uploadBtn','exportBtn','fileInput','welcomeCards','canvasNotice',
  ...
  'addNodeBtn','nodeMenu','nodeMenuContent','nodeContextMenu','mouseTools','providerSettingsBtn','providerModal','providerModalClose','providerSettingsForm','providerSaveBtn','providerSaveStatus','assetDrawerBtn','assetDrawer','assetFilterChips','inspectorDrawer','timelineToggleBtn','timelineShell','timelineCloseBtn','helpBtn','helpModal','helpModalClose','textOutputModal','textOutputTitle','textOutputMeta','textOutputTable','textOutputEditor','textOutputContinuityBtn','textOutputStoryboardBtn','textOutputEditBtn','textOutputSaveBtn','textOutputModalClose','textOutputCopyBtn','agentBtn','agentDrawer','agentNewBtn','agentClearChatBtn','agentAttachBtn','agentConfirmBtn','agentModelSelect','agentMessages','agentProposal','agentInput','agentSkillList','agentSendBtn','agentCancelBtn'
...
const S = {
  ...
  agentModels:[], agentSession:null, agentRun:null, agentQuality:null, agentController:null, agentRunStop:false, agentStage:'plan', agentChange:null, agentShowShots:false, agentManualConfirm:false, assetPick:null, assetFilter:{tag:'all',kind:'all'}, uploadTargetNodeId:null, textOutputNodeId:null, textOutputDirect:null, textOutputOriginal:'', canvasNotice:null, canvasNoticeTimer:null,
};
...
async function openProject(projectId){
  ...
  S.workflow={version:2,nodes:workflow.nodes||[],edges:workflow.edges||[],sequence:workflow.sequence||null};
  ...
  normalizeTimeline(); renderProjectSelect(); renderAll(); resumeNodeJobs(); if(promptMigrated||presetPromptMigrated||layoutMigrated||referencesMigrated||aspectMigrated||storyboardLayoutMigrated)scheduleSave(); requestAnimationFrame(()=>{if(arrangeAllStoryboardNodes(true)){scheduleSave();renderAll();}}); setTimeout(fitCanvas,0); if(els.agentDrawer&&!els.agentDrawer.classList.contains('hidden-drawer'))loadAgent();
}
...
function migrateImagePresetPrompts(){let changed=false;for(const n of S.workflow.nodes.filter(n=>n.type==='imageGen'&&n.data?.imagePresetId)){const preset=imagePresetById(n.data.imagePresetId),source=String(n.data.presetSourcePrompt||'参考图片中的主体与场景').trim();if(!preset)continue;const systemPrompt=composeImagePresetPrompt(preset,source),current=String(n.data.prompt||'').trim();if(n.data.presetSystemPrompt!==systemPrompt){n.data.presetSystemPrompt=systemPrompt;changed=true;}if(current===systemPrompt||current===source){n.data.prompt='';changed=true;}}return changed;}
...
function phaseLabel(phase) {
  return ({ preparing: '准备中', generating: '生成中', retrying: '网络重试', downloading: '下载中', finalizing: '收尾中', rate_limited: '限流等待', provider_busy: '视频服务繁忙' })[phase] || '';
}
...
function nodeStatusBadge(n){const s=n.data.status||'idle',p=Math.max(0,Math.min(100,Number(n.data.progress||0))),ph=n.data.phase,elapsed=n.data.progressStartedAt?` · ${progressElapsedLabel(n.data.progressStartedAt)}`:'';if(s==='queued')return `<span class="node-status-badge run">${isRetryWaitPhase(ph)?retryWaitLabel(ph):'排队中'} ${p}%${elapsed}</span>`;if(s==='processing'||s==='running')return `<span class="node-status-badge run">${ph?`${phaseLabel(ph)} `:`生成中 `}${p}%${elapsed}</span>`;if(s==='failed')return `<span class="node-status-badge failed">失败</span>`;if(s==='canceled')return `<span class="node-status-badge canceled">已取消</span>`;return '';}
function nodeProgressMarkup(n){const s=n.data.status||'idle',active=['queued','processing','running'].includes(s);if(!active)return '';const p=Math.max(0,Math.min(100,Number(n.data.progress||0)));return `<div class="node-progress" role="progressbar" aria-label="生成进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${p}"><span style="width:${p}%"></span></div>`;}
...
function nodeContextToolbar(n){const out=generationPreviewAsset(n,'image')||generationPreviewAsset(n,'video'),label=out?.kind==='video'?'视频':'图片',presets=imagePresetToolbar(n);return `<div class="node-context-toolbar" role="toolbar" aria-label="节点操作">${presets}${presets?'<span class="node-context-divider" aria-hidden="true"></span>':''}<button data-node-toolbar="duplicate" title="复制节点" aria-label="复制节点">${icon('copy')}</button><button data-node-toolbar="inspect" title="打开属性" aria-label="打开属性">${icon('adjustments-horizontal')}</button>${out?`<button data-action="openMediaOutput" title="放大${label}" aria-label="放大${label}">${icon('arrows-maximize')}</button><button data-action="downloadOutput" title="下载${label}" aria-label="下载${label}">${icon('download')}</button>`:''}</div>`;}
...
function nodeTemplate(n){
  ...
  if(n.type==='textGen'){
    ...
    const previewToggle=' data-action="togglePreview" title="展开 / 收起输入区"';
...
function sequenceReviewToolbar(n){if(n.type!=='videoGen'||!n.data.sequenceClipId)return '';if(n.data.sequenceReviewStatus==='accepted')return '<div class="sequence-review-toolbar is-accepted" role="status">本段已验收</div>';if(n.data.sequenceReviewStatus==='rejected')return '<div class="sequence-review-toolbar is-rejected" role="status">本段已拒绝，请重新生成</div>';if(n.data.status!=='succeeded')return '';return '<div class="sequence-review-toolbar" role="group" aria-label="本段验收"><span>本段生成完成</span><button class="primary" data-action="sequenceAccept">接受并继续</button><button data-action="sequenceReject">拒绝</button></div>';}
...
function renderEdges(){
  ...
  for(const e of S.workflow.edges){const a=nodeById(e.source),b=nodeById(e.target);if(!a||!b)continue;const p1=worldToScreen(portWorld(a,'out')),p2=worldToScreen(portWorld(b,'in'));const c=Math.max(55,Math.abs(p2.x-p1.x)*.42);const d=`M ${p1.x} ${p1.y} C ${p1.x+c} ${p1.y}, ${p2.x-c} ${p2.y}, ${p2.x} ${p2.y}`;const st=edgeState(e),sel=e.id===S.selectedEdgeId;const color=st==='failed'?'var(--danger)':(sel||st==='done')?'var(--edge-blue,#8fcce9)':'#63738b';const cls=st==='failed'?'failed':st==='done'?'satisfied':'pending';addPath(d,`edge-path edge-glow ${cls}`,e.id).setAttribute('style',`color:${color}`);addPath(d,`edge-path edge-main ${cls}${sel?' edge-selected':''}`,e.id).setAttribute('style',`color:${color}`);if(st==='done'){const light=addPath(d,'edge-light',e.id);light.setAttribute('pathLength','100');}const hit=addPath(d,'edge-hit',e.id);hit.addEventListener('pointerdown',ev=>{ev.stopPropagation();if(S.selectedEdgeId===e.id)selectEdge(null);else selectEdge(e.id);});}
...
function fitCanvas(){fitNodes(S.workflow.nodes);}
  ...
  function autoLayoutNodes(){
    ...
    const GAP_X = NODE_GAP_X, GAP_Y = 50, START_X = 50, START_Y = 50;
...
async function generateNode(nodeId){
  ...
  n.data.previousOutputAssetIds=[...nodeOutputAssets(n)];n.data.jobId='';n.data.status='queued';n.data.phase='queued';n.data.progress=0;n.data.progressStartedAt=Date.now();n.data.error='';n.data.providerAttempt=0;n.data.providerMaxAttempts=0;n.data.variantAssetIds=[];n.data.selectedVariantIndex=undefined;n.data.sequenceReviewStatus='';renderNode(n);
  try{
...
function addAssetToTimeline(assetId,{sequenceClipId='',sourceNodeId=''}={}){const a=findAsset(assetId);if(!a)return;const linked=sequenceClipId?S.workflow.sequence?.clips?.find(value=>value.id===sequenceClipId):S.workflow.sequence?.clips?.find(value=>{const video=nodeById(value.videoNodeId);return nodeOutputAssets(video).includes(assetId);});sequenceClipId=linked?.id||sequenceClipId;sourceNodeId=sourceNodeId||linked?.videoNodeId||'';beginTimelineSnapshot();const fps=S.timeline.fps||30;const track=a.kind==='audio'?'A1':'V1';const duration=Math.max(1,a.kind==='image'?fps*3:Math.round((a.durationMs||3000)/1000*fps));const end=Math.max(0,...S.timeline.items.filter(i=>i.track===track).map(i=>i.startFrame+i.durationInFrames));const item={id:id(),track,startFrame:end,durationInFrames:duration,name:a.filename,kind:a.kind,sourceAssetId:a.id,sourceNodeId,sequenceClipId,sourceInFrame:0,sourceOutFrame:duration,playbackRate:1,volume:1,opacity:1,fadeInFrames:0,fadeOutFrames:0,transform:{x:0,y:0,scale:1}};S.timeline.items.push(item);if(sequenceClipId){const clip=S.workflow.sequence?.clips?.find(value=>value.id===sequenceClipId);if(clip){clip.timelineItemId=item.id;clip.timelineStatus='added';}}S.selectedClipId=item.id;S.selectedNodeId=S.selectedEdgeId=null;S.selectedNodeIds=[];scheduleTimelineSave();if(sequenceClipId)scheduleSave();renderTimeline();renderInspector();renderPreview();toast('已加入 Timeline');}
...
function closeControlDropdowns(except=null){$$('.generator-mode-menu[open],.generator-composer .node-advanced[open],.image-preset-menu[open]').forEach(menu=>{if(menu!==except)menu.removeAttribute('open');});}
...
const AGNES_MODEL_FIELDS=[['文本','AGNES_TEXT_MODEL'],['Agent','AGNES_AGENT_MODEL'],['图片','AGNES_IMAGE_MODEL'],['视频','AGNES_VIDEO_MODEL']];
const APIMART_MODEL_GROUPS=[['text','文本'],['agent','Agent'],['image','图片'],['video','视频']];
const PROVIDER_MODEL_FIELDS={
  agnes:AGNES_MODEL_FIELDS.map(([title,key])=>[title,key,[]]),
  deepseek:[['文本','DEEPSEEK_TEXT_MODEL',['deepseek-v4-pro','deepseek-v4-flash']],['Agent','DEEPSEEK_AGENT_MODEL',['deepseek-v4-pro','deepseek-v4-flash']]],
  bailian:[['文本','BAILIAN_TEXT_MODEL',['qwen-plus','qwen-max','qwen-turbo','qwen3-max','qwen3-plus','qwen3-coder-plus']],['Agent','BAILIAN_AGENT_MODEL',['qwen-plus','qwen-max','qwen-turbo','qwen3-max','qwen3-plus','qwen3-coder-plus']],['图片','BAILIAN_IMAGE_MODEL',['qwen-image-3.0-pro','qwen-image-3.0','qwen-image-2.0-pro','qwen-image-2.0','qwen-image-max','qwen-image-plus','qwen-image']],['视频','BAILIAN_VIDEO_MODEL',['wan2.7-t2v-2026-06-12','wan2.7-t2v','wan3.0-video']]],
};
...
function modelSelectHtml(key,value,items,attrs=''){const options=[...new Set([value,...items].filter(Boolean))];return`<label><select data-provider-key="${esc(key)}" ${attrs}>${options.map(item=>`<option value="${esc(item)}" ${item===value?'selected':''}>${esc(item)}</option>`).join('')}</select></label>`;}
...
function staticProviderModelsHtml(providerId,data){const fields=PROVIDER_MODEL_FIELDS[providerId]||[];return`<div class="provider-models"><div class="provider-models-head"><strong>模型分类</strong><span>${fields.length} 类</span></div><div class="provider-model-grid">${fields.map(([title,key,options])=>{const dynamic=providerId==='agnes'?agnesModelOptions(data,title==='Agent'?'agent':title==='图片'?'image':title==='视频'?'video':'text'):options;return`<section class="provider-model-group"><div class="provider-model-group-title">${esc(title)}</div>${modelSelectHtml(key,S.providerSettings[key]||'',dynamic)}</section>`;}).join('')}</div></div>`;}
...
function sequenceClipNodes(clip){return S.workflow.nodes.filter(node=>[clip?.imageNodeId,clip?.videoNodeId].includes(node.id));}
function sequenceVideoNode(clip){return nodeById(clip?.videoNodeId);}
function sequenceImageNode(clip){return nodeById(clip?.imageNodeId);}
function focusSequenceClip(clipId){const sequence=S.workflow?.sequence,clip=sequence?.clips?.find(item=>item.id===clipId),nodes=sequenceClipNodes(clip);if(!clip||!nodes.length)return toast('这个镜头还没有创建节点');S.selectedNodeId=clip.videoNodeId||clip.imageNodeId;S.selectedNodeIds=nodes.map(node=>node.id);S.selectedEdgeId=null;S.selectedClipId=null;updateCanvasSelectionDom();renderEdges();renderInspector();requestAnimationFrame(()=>fitNodes(nodes));}
function agentClipLabel(clip){return clip.timelineStatus==='added'?'已加入时间线':clip.status==='accepted'?'已接受':clip.lastVerdict==='rejected'?'待修改':clip.status==='ready'?'当前段':'待创建';}
function agentRunHtml(run){if(!run)return'';const running=['queued','running','paused'].includes(run.status),label={queued:'待执行',running:'执行中',paused:'已暂停',completed:'已完成',failed:'有失败任务',canceled:'已取消'}[run.status]||run.status;return `<section class="agent-run-card"><div><strong>Agent Run</strong><span>${label} · ${run.completed}/${run.total}</span></div><div class="progressbar"><span style="width:${run.progress||0}%"></span></div><div class="agent-run-actions">${running?`<button data-agent-run-action="${run.status==='paused'?'resume':'pause'}">${run.status==='paused'?'继续':'暂停'}</button><button class="danger" data-agent-run-action="cancel">取消</button>`:''}${run.status==='failed'?'<button data-agent-run-action="retry">重试失败任务</button>':''}</div></section>`;}
function agentFlowStatus(sequence,pending){if(pending)return['1','确认方案'];if(!sequence?.plan)return['1','输入创意'];if(S.agentRun&&['queued','running','paused'].includes(S.agentRun.status))return['3',S.agentRun.status==='paused'?'已暂停生成':'正在生成'];const active=sequence.clips.find(clip=>clip.id===sequence.activeClipId),video=sequenceVideoNode(active);if(!active)return['4','全部完成'];if(video?.data?.status==='succeeded')return['3','验收当前镜头'];if(sequence.clips.some(clip=>clip.status==='accepted'))return['3',`继续 S${active.index+1}`];return['2',`准备 S${active.index+1}`];}
const SEEDANCE_WORKFLOWS=[
  {icon:'◆',title:'产品广告',meta:'产品参考 → 关键帧 → 首帧视频',prompt:'使用 Seedance 产品广告工作流，为【产品/主题】规划一支 30 秒短片。先定义产品外观与核心卖点；每镜先生成关键帧图片，再以该图作为首帧生成视频。要求产品外观、材质、颜色和比例跨镜一致；镜头包含产品特写、真实使用场景和品牌收尾，并写明每镜的动作、运镜、声音与结尾状态。'},
  {icon:'↔',title:'首尾帧转场',meta:'起始画面 + 结束画面 → 转场镜头',prompt:'使用 Seedance 首尾帧工作流，为【主题】设计一个可衔接的转场镜头。先说明起始画面、结束画面与中间唯一动作；将两张关键帧分别作为首帧和尾帧参考。保持主体身份、场景空间、光线和道具连续，明确镜头如何覆盖动作过程与最终停留状态。'},
  {icon:'◎',title:'连续叙事',meta:'分镜 → 首镜 → 验收 → 下一镜',prompt:'使用 Seedance 连续叙事工作流，为【故事/主题】规划一组连贯短片。先输出完整镜头表，但只执行首镜；每镜都要写清上一个已验收镜头的结尾状态、当前镜头的叙事任务、可见动作、运镜、声音和新的结尾状态。角色、服装、道具、场景和光线必须持续一致。'},
  {icon:'✦',title:'素材改造',meta:'参考素材 → 修改目标 → 重拍/续写',prompt:'使用 Seedance 素材改造工作流，基于【已有图片/视频和修改目标】制定重拍或续写方案。先识别必须保留的主体、产品、构图与光线，再只改变用户指定的内容；如果是视频，说明采用重拍还是尾帧续写，并写出前后状态、动作衔接和声音要求。'}
];
function renderAgentSkills(){if(!els.agentSkillList)return;els.agentSkillList.innerHTML=SEEDANCE_WORKFLOWS.map(workflow=>`<button type="button" class="agent-skill-item" data-agent-template="${esc(workflow.prompt)}"><i>${esc(workflow.icon)}</i><span><b>${esc(workflow.title)}</b><small>${esc(workflow.meta)}</small></span></button>`).join('');$$('[data-agent-template]',els.agentSkillList).forEach(button=>button.addEventListener('click',e=>{els.agentInput.value=e.currentTarget.dataset.agentTemplate;els.agentInput.focus();}));}
function agentTemplateHtml(){return '';}
function agentProjectHtml(sequence,pending){if(!sequence?.plan)return pending?'':agentTemplateHtml();const b=sequence.plan.brief,active=sequence.clips.find(clip=>clip.id===sequence.activeClipId),flow=agentFlowStatus(sequence,pending);return `<section class="agent-project-card"><div class="agent-project-head"><strong>${esc(b.title)}</strong><span>${sequence.clips.length} 镜头 · ${esc(b.targetDurationSec)}s · ${esc(b.aspectRatio)}</span></div><div class="agent-flow-status"><span>步骤 ${flow[0]}/4</span><strong>${esc(flow[1])}</strong></div><small>${active?`当前 S${active.index+1} · ${esc(sequence.plan.shots?.[active.index]?.title||'镜头')}`:'序列已完成'}</small><div class="agent-project-actions"><button class="primary" data-agent-run-mode="first-shot">${active?'执行当前镜头':'开始执行'}</button><details class="agent-more-actions"><summary>更多</summary><div><button data-agent-run-mode="all">批量执行</button><button data-agent-quality="1">检查质量</button></div></details></div>${agentRunHtml(S.agentRun)}${S.agentQuality?`<div class="agent-quality ${esc(S.agentQuality.status)}">质量 ${S.agentQuality.score}分：${esc(S.agentQuality.issues?.join('；')||'基础检查通过')}<small>${esc(S.agentQuality.suggestions?.join('；')||'')}</small></div>`:''}</section>`;}
function agentSequenceHtml(sequence){if(!sequence?.clips?.length)return'';const accepted=sequence.clips.filter(clip=>clip.status==='accepted').length;return `<details class="agent-sequence-card"><summary class="agent-sequence-head"><strong>镜头进度</strong><span>${accepted}/${sequence.clips.length} 已完成</span></summary><div class="agent-sequence-list">${sequence.clips.map(clip=>`<button type="button" class="agent-sequence-row ${esc(clip.status)} ${clip.lastVerdict==='rejected'?'rejected':''}" data-agent-clip="${esc(clip.id)}"><span>S${clip.index+1}</span><b>${agentClipLabel(clip)}</b>${clip.observedEndState?`<small>结尾：${esc(clip.observedEndState)}</small>`:clip.revisionNote?`<small>修改：${esc(clip.revisionNote)}</small>`:''}</button>`).join('')}</div></details>`;}
function agentCurrentClipHtml(sequence){const clip=sequence?.clips?.find(item=>item.id===sequence.activeClipId);if(!sequence?.clips?.length)return'';if(!clip)return`<section class="agent-current-card"><div><strong>当前段</strong><span>全部镜头已完成</span></div></section>`;const shot=sequence.plan?.shots?.[clip.index]||{},image=sequenceImageNode(clip),video=sequenceVideoNode(clip),videoDone=video?.data?.status==='succeeded'&&nodeOutputAssets(video).length,imageDone=image?.data?.status==='succeeded'&&nodeOutputAssets(image).length,videoBusy=['queued','processing'].includes(video?.data?.status),imageBusy=['queued','processing'].includes(image?.data?.status),videoAsset=nodeOutputAssets(video).find(Boolean);return `<section class="agent-current-card"><div class="agent-current-head"><div><strong>当前段 S${String(clip.index+1).padStart(2,'0')} · ${esc(shot.title||'镜头')}</strong><span>${clip.timelineStatus==='added'?'已加入时间线':clip.lastVerdict==='rejected'?'待修改':videoDone?'等待验收':videoBusy?'视频生成中':imageDone?'等待生成视频':imageBusy?'图片生成中':'等待生成图片'}</span></div><button data-agent-action="locate" data-agent-clip="${esc(clip.id)}">定位节点</button></div><p>${esc(shot.purpose||'')}</p><div class="agent-current-actions"><button data-agent-action="generateImage" data-agent-clip="${esc(clip.id)}" ${imageBusy?'disabled':''}>${imageDone?'重新生成图片':'生成图片'}</button><button data-agent-action="generateVideo" data-agent-clip="${esc(clip.id)}" ${videoBusy?'disabled':''}>${videoDone?'重新生成视频':'生成视频'}</button>${videoDone?`<button class="primary" data-agent-action="accept" data-agent-clip="${esc(clip.id)}">接受并继续</button><button data-agent-action="reject" data-agent-clip="${esc(clip.id)}">修改当前段</button>${videoAsset?`<button data-agent-action="timeline" data-asset-id="${esc(videoAsset)}">${clip.timelineStatus==='added'?'重新定位时间线':'加入时间线'}</button>`:''}`:''}</div>${clip.revisionNote?`<small class="agent-current-note">修改意见：${esc(clip.revisionNote)}</small>`:''}</section>`;}
function agentProposalMarkdown(proposal,kind='full'){const p=proposal.plan,b=p.brief,s=p.styleBible,rows=p.shots.map((shot,i)=>`| ${i+1} | ${shot.title} | ${shot.imagePrompt} | ${shot.videoPrompt} | ${shot.purpose} | ${shot.durationSec}s | ${shot.audioNote||'—'} |`).join('\n'),brief=`# ${b.title}\n\n## 创作简报\n\n- 受众 / 平台：${b.audience} · ${b.platform}\n- 基调 / 结尾：${b.tone} · ${b.ending}\n- 视觉：${s.visualStyle} · ${s.palette}\n- 时长 / 画幅：${b.targetDurationSec}s · ${b.aspectRatio}\n\n${b.logline}\n`;return kind==='script'?brief:kind==='storyboard'?`${brief}\n## 正式分镜拆解\n\n| 镜号 | 阶段/景别 | 图片提示词 | 视频提示词 | 文案/旁白 | 时长 | 音效/音乐 |\n| --- | --- | --- | --- | --- | --- | --- |\n${rows}\n`:`${brief}\n## 分镜表\n\n${rows}\n`;}
function agentProposalHtml(proposal){if(!proposal)return'';const title=proposal.plan.brief.title,status=proposal.status==='pending'?'待确认':proposal.status==='applied'?'已应用':'已取代';return `<section class="agent-file-output"><div class="agent-file-label"><span>▤</span>创作文本已保存</div><button class="agent-file-row" data-agent-file="${esc(proposal.id)}:script"><span class="agent-file-icon">▤</span><span>${esc(title)}-创作简报.md</span><small>${status}</small></button><button class="agent-file-row" data-agent-file="${esc(proposal.id)}:storyboard"><span class="agent-file-icon">▤</span><span>${esc(title)}-分镜表.md</span><small>${status}</small></button>${proposal.status==='pending'?`<button class="primary agent-apply" data-agent-apply="${esc(proposal.id)}">确认方案并创建首镜</button>`:''}</section>`;}
...
function agentInputPlaceholder(){const sequence=S.workflow?.sequence,clip=sequence?.clips?.find(item=>item.id===sequence.activeClipId),video=sequenceVideoNode(clip);if(!sequence)return'描述短片创意，例如：帮我做一个世界杯短片';if(clip?.lastVerdict==='rejected')return'补充当前段修改意见，然后点“重新生成视频”';if(video?.data?.status==='succeeded')return'描述本段结尾状态，或写下哪里要改';return'和 Agent 讨论当前方案，或描述要调整的方向';}
function agentStageForState(pending,sequence){if(S.agentRun&&['queued','running','paused'].includes(S.agentRun.status))return'run';if(pending)return'plan';if(sequence?.clips?.length)return'shots';return'plan';}
function agentChangeHtml(){const change=S.agentChange;if(!change)return'';return`<section class="agent-change-card"><div><strong>${esc(change.title)}</strong><span>已同步到画布</span></div><p>${esc(change.detail)}</p><div class="agent-change-actions"><button data-agent-action="locate-change">定位变更</button><button data-agent-action="undo-change">撤销本次变更</button></div></section>`;}
function agentChatHtml(session){const messages=session.messages||[];return messages.length?messages.map(agentMessageHtml).join(''):'<div class="agent-empty"><strong>开始一个创作任务</strong><span>告诉我想做什么，我会在这里生成脚本、分镜和素材。</span></div>';}
function agentStageHtml(stage,session,pending,sequence){if(stage==='discussion')return agentChatHtml(session);if(stage==='plan')return`${agentProjectHtml(sequence,pending)}${agentChangeHtml()}${pending?agentProposalHtml(pending):agentChatHtml(session)}`;if(stage==='shots')return`${sequence?agentProjectHtml(sequence,null):''}${agentSequenceHtml(sequence)}${agentCurrentClipHtml(sequence)}${agentChangeHtml()}`;return`${sequence?agentProjectHtml(sequence,null):''}${agentRunHtml(S.agentRun)}${agentCurrentClipHtml(sequence)}${agentChangeHtml()}`;}
function openAgentMarkdownPage(fileKey){const [proposalId,kind='full']=String(fileKey).split(':'),proposal=S.agentSession?.proposals?.find(item=>item.id===proposalId);if(!proposal)return;const title=String(proposal.plan?.brief?.title||'创作简报'),text=agentProposalMarkdown(proposal,kind);if(kind==='storyboard')return openDirectTextOutputPage({title,preset:'storyboard',text,render:value=>renderMarkdownTables(value,false,'storyboard')});return openDirectTextOutputPage({title,text,render:renderAgentBriefDocument});}
function agentWelcomeHtml(){return`<section class="agent-welcome"><div class="agent-welcome-mark">✦</div><h2>开始一个创作任务</h2><p>描述你的创意，或从输入框下方的 Skills 选择工作流。</p></section>`;}
function agentContextHtml(sequence,pending){if(pending){const b=pending.plan?.brief||{};return`<section class="agent-context-card"><div class="agent-context-head"><strong>${esc(b.title||'创作方案')}</strong><span>待确认</span></div><p>${esc(b.logline||'Agent 已经整理出一个创作方向，请先确认或继续修改。')}</p><div class="agent-context-meta">${esc(b.aspectRatio||'16:9')} · ${esc(b.targetDurationSec||'—')} 秒 · ${esc(b.platform||'视频')}</div><div class="agent-context-actions"><button class="primary agent-apply" data-agent-apply="${esc(pending.id)}">确认方案并创建首镜</button><button data-agent-file="${esc(pending.id)}:storyboard">查看分镜</button></div></section>`;}if(!sequence?.clips?.length)return'';const active=sequence.clips.find(clip=>clip.id===sequence.activeClipId),shot=sequence.plan?.shots?.[active?.index]||{},video=sequenceVideoNode(active),status=S.agentRun&&['queued','running','paused'].includes(S.agentRun.status)?'执行中':video?.data?.status==='succeeded'?'待复核':active?'待执行':'已完成';return`<section class="agent-context-card"><div class="agent-context-head"><strong>${esc(sequence.plan?.brief?.title||'当前创作')}</strong><span>${status}</span></div><p>${active?`当前镜头 S${active.index+1} · ${esc(shot.title||'镜头')}`:'全部镜头已完成'}</p><div class="agent-context-meta">${sequence.clips.length} 个镜头 · ${esc(sequence.plan?.brief?.aspectRatio||'16:9')} · ${esc(sequence.plan?.brief?.targetDurationSec||'—')} 秒</div><div class="agent-context-actions"><button class="primary" data-agent-run-mode="first-shot">${status==='执行中'?'查看执行进度':active?'执行当前镜头':'查看结果'}</button><button data-agent-action="show-shots">${S.agentShowShots?'收起镜头':'查看镜头'}</button></div>${S.agentShowShots?`${agentSequenceHtml(sequence)}${agentCurrentClipHtml(sequence)}`:''}</section>`;}
function agentConversationHtml(session,sequence,pending){const messages=agentChatHtml(session);return`${messages}${agentContextHtml(sequence,pending)}`;}
function renderAgent(){const session=S.agentSession||{messages:[],proposals:[]},pending=[...session.proposals].reverse().find(proposal=>proposal.status==='pending'),sequence=S.workflow?.sequence,header=els.agentDrawer?.querySelector('.agent-brand strong');if(header)header.textContent='Agent';const hasState=session.messages.length||pending||sequence?.clips?.length;els.agentMessages.innerHTML=hasState?agentConversationHtml(session,sequence,pending):agentWelcomeHtml();els.agentInput.placeholder=agentInputPlaceholder();const root=els.agentMessages;$$('[data-agent-template]',root).forEach(button=>button.addEventListener('click',e=>{els.agentInput.value=e.currentTarget.dataset.agentTemplate;els.agentInput.focus();}));$$('[data-agent-file]',root).forEach(button=>button.addEventListener('click',e=>openAgentMarkdownPage(e.currentTarget.dataset.agentFile)));$$('[data-agent-apply]',root).forEach(button=>button.addEventListener('click',e=>applyAgentProposalUI(e.currentTarget.dataset.agentApply)));$$('[data-agent-run-mode]',root).forEach(button=>button.addEventListener('click',e=>startAgentRun(e.currentTarget.dataset.agentRunMode)));$$('[data-agent-action]',root).forEach(button=>button.addEventListener('click',e=>handleAgentAction(e.currentTarget.dataset.agentAction,e.currentTarget)));renderAgentSkills();renderAgentModels();els.agentMessages.scrollTop=els.agentMessages.scrollHeight;}
...
async function loadAgent(){els.agentMessages.innerHTML='<div class="agent-empty">正在读取本地会话…</div>';try{const [models,session,run]=await Promise.all([api('/api/agent/models'),api(`/api/projects/${S.projectId}/agent`),api(`/api/projects/${S.projectId}/agent/run`)]);S.agentModels=models.models||[];S.agentSession=session;S.agentRun=run.run;S.agentQuality=null;renderAgent();}catch(e){els.agentMessages.innerHTML=`<div class="agent-empty error-text">${esc(e.message)}</div>`;}}
...
function setAgentBusy(busy){els.agentSendBtn.disabled=busy||!S.agentModels.length;els.agentModelSelect.disabled=busy;els.agentNewBtn.disabled=busy;els.agentClearChatBtn.disabled=busy;els.agentCancelBtn.classList.toggle('hidden',!busy);els.agentInput.disabled=busy;}
...
async function checkAgentQuality(){try{S.agentQuality=await api(`/api/projects/${S.projectId}/agent/quality`);renderAgent();}catch(e){toast(e.message,'error');}}
async function controlAgentRun(action){if(!S.agentRun)return;if(action==='retry'){return startAgentRun(S.agentRun.mode);}try{if(action==='cancel'){const activeTask=S.agentRun.tasks.find(task=>['queued','processing'].includes(nodeById(task.nodeId)?.data?.status)),activeNode=activeTask&&nodeById(activeTask.nodeId);if(activeNode?.data?.jobId)await cancelNodeGeneration(activeNode);}const result=await api(`/api/projects/${S.projectId}/agent/run/${S.agentRun.id}?action=${action}`,{method:'POST'});S.agentRun=result.run;S.agentRunStop=action==='cancel';renderAgent();if(action==='resume')executeAgentRun(S.agentRun.id);}catch(e){toast(e.message,'error');}}
async function startAgentRun(mode='first-shot'){try{const result=await api(`/api/projects/${S.projectId}/agent/run`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mode})});S.agentRun=result.run;S.agentRunStop=false;S.agentStage='run';renderAgent();executeAgentRun(S.agentRun.id);}catch(e){toast(e.message,'error');}}
async function executeAgentRun(runId){const run=S.agentRun;if(!run||run.id!==runId)return;await api(`/api/projects/${S.projectId}/agent/run/${runId}?action=resume`,{method:'POST'}).catch(()=>{});for(const task of run.tasks){if(S.agentRunStop)return;let current=(await api(`/api/projects/${S.projectId}/agent/run`)).run;if(!current||current.id!==runId||['canceled','completed','failed'].includes(current.status))return;while(current.status==='paused'){await new Promise(resolve=>setTimeout(resolve,700));current=(await api(`/api/projects/${S.projectId}/agent/run`)).run;if(!current||current.status==='canceled')return;}const node=nodeById(task.nodeId);if(!node){await api(`/api/projects/${S.projectId}/agent/run/${runId}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({taskId:task.id,status:'failed',error:'节点不存在'})});continue;}if(node.data.status==='succeeded'){await api(`/api/projects/${S.projectId}/agent/run/${runId}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({taskId:task.id,status:'succeeded'})});continue;}const result=await generateNode(node.id);await api(`/api/projects/${S.projectId}/agent/run/${runId}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({taskId:task.id,status:result?.ok?'succeeded':'failed',error:result?.ok?'':'生成失败'})});S.agentRun=(await api(`/api/projects/${S.projectId}/agent/run`)).run;renderAgent();if(!result?.ok&&run.mode==='first-shot')return;}S.agentRun=(await api(`/api/projects/${S.projectId}/agent/run`)).run;if(run.mode==='all'&&S.agentRun.status==='completed'){for(const task of run.tasks.filter(item=>item.type==='video')){const node=nodeById(task.nodeId),asset=nodeOutputAssets(node).map(findAsset).find(Boolean);if(asset&&!S.timeline.items.some(item=>item.sourceAssetId===asset.id))addAssetToTimeline(asset.id);}await saveTimeline();}renderAgent();}
async function handleAgentAction(action,button){const sequence=S.workflow?.sequence,clip=sequence?.clips?.find(item=>item.id===button.dataset.agentClip||item.id===sequence.activeClipId),image=sequenceImageNode(clip),video=sequenceVideoNode(clip);if(action==='show-shots'){S.agentShowShots=!S.agentShowShots;return renderAgent();}if(action==='locate')return focusSequenceClip(clip?.id);if(action==='locate-change'){const nodes=(S.agentChange?.nodeIds||[]).map(nodeById).filter(Boolean);if(nodes.length){S.selectedNodeIds=nodes.map(node=>node.id);S.selectedNodeId=nodes[0].id;updateCanvasSelectionDom();renderEdges();renderInspector();requestAnimationFrame(()=>fitNodes(nodes));}return;}if(action==='undo-change'){canvasUndo();S.agentChange=null;S.agentStage='plan';return renderAgent();}if(action==='timeline')return addAssetToTimeline(button.dataset.assetId);if(action==='generateImage'&&image){S.agentStage='run';focusSequenceClip(clip.id);await generateNode(image.id);return renderAgent();}if(action==='generateVideo'&&video){S.agentStage='run';focusSequenceClip(clip.id);await generateNode(video.id);return renderAgent();}if(action==='accept'&&video){S.agentStage='shots';return reviewSequenceClip(video,'accept',{observedEndState:els.agentInput.value.trim(),fromAgentPanel:true});}if(action==='reject'&&video){const note=els.agentInput.value.trim();if(!note){els.agentInput.placeholder='先写当前段需要修改的地方，再点“修改当前段”';els.agentInput.focus();return toast('请先在输入框写修改意见','error');}S.agentStage='shots';return reviewSequenceClip(video,'reject',{revisionNote:note,fromAgentPanel:true});}}
async function reviewSequenceClip(n,decision,provided={}){const body={decision};if(decision==='accept'){let note=provided.observedEndState;if(note==null){note=prompt('可选：描述本段结尾状态，用于下一段连续性承接。','');if(note===null)return;}body.observedEndState=String(note||'').trim();}else if(decision==='reject'){let note=provided.revisionNote;if(note==null){note=prompt('输入当前段需要修改的地方，会追加到本段视频提示词。','');if(note===null)return;}body.revisionNote=String(note||'').trim();}const buttons=$$('.sequence-review-toolbar button',els.canvasWorld.querySelector(`[data-id="${n.id}"]`));buttons.forEach(button=>button.disabled=true);try{const acceptedAsset=decision==='accept'?nodeOutputAssets(n).map(findAsset).find(asset=>asset?.kind==='video'):null;const result=await api(`/api/projects/${S.projectId}/sequence/clips/${n.data.sequenceClipId}/review`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});beginCanvasSnapshot();S.workflow=result.workflow;S.selectedNodeId=result.appliedNodeIds?.[0]||n.id;S.selectedNodeIds=S.selectedNodeId?[S.selectedNodeId]:[];S.selectedEdgeId=null;if(provided.fromAgentPanel)els.agentInput.value='';renderAll();if(acceptedAsset&&!S.timeline.items.some(item=>item.sourceAssetId===acceptedAsset.id))addAssetToTimeline(acceptedAsset.id);if(!els.agentDrawer.classList.contains('hidden-drawer'))renderAgent();if(result.appliedNodeIds?.length)requestAnimationFrame(()=>fitNodes(S.workflow.nodes.filter(node=>result.appliedNodeIds.includes(node.id))));toast(decision==='accept'?(result.nextClipId?'已接受本段，下一段已创建并加入时间线':'已接受，序列已完成'):'已记录修改意见，请重新生成当前段');}catch(e){buttons.forEach(button=>button.disabled=false);toast(friendlyError(e.message),'error');}}
async function applyAgentProposalUI(proposalId){const button=els.agentMessages.querySelector('[data-agent-apply]');if(button){button.disabled=true;button.textContent='同步到画布中…';}try{const result=await api(`/api/projects/${S.projectId}/agent/proposals/${proposalId}/apply`,{method:'POST'});beginCanvasSnapshot();S.workflow=result.workflow;S.selectedNodeId=result.briefNodeId||result.appliedNodeIds?.[0]||null;S.selectedNodeIds=S.selectedNodeId?[S.selectedNodeId]:[];S.selectedEdgeId=null;S.agentStage='shots';S.agentChange={title:'方案已写入画布',detail:`新增 ${result.appliedNodeIds?.length||0} 个节点，并建立了首镜工作流。你可以先审阅镜头，再开始生成。`,nodeIds:result.appliedNodeIds||[]};const local=S.agentSession?.proposals?.find(p=>p.id===proposalId);if(local)Object.assign(local,result.proposal);renderAll();renderAgent();requestAnimationFrame(()=>fitNodes(S.workflow.nodes.filter(n=>result.appliedNodeIds.includes(n.id))));toast('方案已确认，镜头工作台已打开');}catch(e){toast(e.message,'error');if(button){button.disabled=false;button.textContent='确认方案并创建首镜';}}}
async function clearAgentChat(){if(!confirm('只清空聊天记录，保留当前方案、进度和画布节点。'))return;S.agentController?.abort();S.agentSession=await api(`/api/projects/${S.projectId}/agent?mode=chat`,{method:'DELETE'});renderAgent();}
async function resetAgentCreative(){if(!confirm('新建创意会清空 Agent 方案和连续项目进度，但不会删除画布上已创建节点。是否继续？'))return;S.agentController?.abort();S.agentRunStop=true;const result=await api(`/api/projects/${S.projectId}/agent?mode=project`,{method:'DELETE'});S.agentSession=result.session;S.agentRun=null;S.agentQuality=null;S.workflow=result.workflow;renderAll();renderAgent();toast('已新建创意，画布节点已保留');}
...
function setupGlobalInteractions(){
  ...
  els.canvas.addEventListener('contextmenu',e=>{const node=e.target.closest('.node'),edge=e.target.closest('.edge-hit');e.preventDefault();if(node){selectNode(node.dataset.id);showCanvasContextMenu(e.clientX,e.clientY);return;}if(edge){selectEdge(edge.dataset.edge);return;}showCanvasContextMenu(e.clientX,e.clientY);});
  ...
  document.addEventListener('pointerdown',e=>{const controlMenu=e.target.closest('.generator-mode-menu,.generator-composer .node-advanced,.image-preset-menu');closeControlDropdowns(controlMenu);if(!e.target.closest('.node-menu')&&!e.target.closest('.context-menu')&&!e.target.closest('#addNodeBtn'))hideMenus();});
}
...
els.agentBtn?.addEventListener('click',openAgent);els.agentSendBtn?.addEventListener('click',sendAgentMessage);els.agentCancelBtn?.addEventListener('click',()=>S.agentController?.abort());els.agentNewBtn?.addEventListener('click',resetAgentCreative);els.agentClearChatBtn?.addEventListener('click',clearAgentChat);els.agentAttachBtn?.addEventListener('click',()=>els.fileInput?.click());els.agentConfirmBtn?.addEventListener('click',()=>{S.agentManualConfirm=!S.agentManualConfirm;els.agentConfirmBtn.setAttribute('aria-pressed',String(S.agentManualConfirm));els.agentConfirmBtn.classList.toggle('active',S.agentManualConfirm);});els.agentInput?.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();sendAgentMessage();}});
