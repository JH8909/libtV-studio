...
const els = Object.fromEntries([
  ...
  'refreshAssetsBtn','assetList','assetLibraryTabs','promptLibraryList','canvas','canvasWorld','edgesLayer','inspector','jobList','fitBtn','zoomOutBtn','zoomInBtn','zoomLabel',
...
async function init(){
  try {
    ...
    renderProjectSelect(); await openProject(S.projects[0].id); setupGlobalInteractions(); bindAssetFilterChips(); bindAssetLibraryTabs();
  } catch(e){ console.error(e); toast(`启动失败：${e.message}`,'error'); }
}

function renderProjectSelect(){
  els.projectSelect.innerHTML=appSelectMarkup({value:S.projectId,options:S.projects.map(p=>({value:p.id,label:p.name})),className:'project-switcher',ariaLabel:'当前项目'});
  $$('[data-app-select-option]',els.projectSelect).forEach(button=>button.addEventListener('click',()=>openProject(button.dataset.appSelectValue)));
}
...
function appSelectMarkup({value='',options=[],className='',ariaLabel='',dataField='',dataProviderKey='',dataApimartSelect=false,dataSelectId=''}){
  ...
  return `<details class="app-select ${className}" ${attrs} data-value="${esc(selected.value)}"><summary aria-expanded="false"><span class="app-select-value">${esc(selected.label)}</span>${icon('chevron-down')}</summary><div class="app-select-popover" role="listbox">${list.map(item=>`<button type="button" role="option" class="app-select-option ${String(item.value)===String(selected.value)?'active':''}" data-app-select-option data-app-select-value="${esc(item.value)}" ${item.disabled?'disabled':''} aria-selected="${String(item.value)===String(selected.value)}">${esc(item.label)}</button>`).join('')}</div></details>`;
}
...
const INTERACTION_DETAILS_SELECTOR='.app-select,.generator-mode-menu,.generator-composer .node-advanced,.generator-param-menu,.image-preset-menu,.agent-skill-menu,.agent-more-actions';
...
function closeOtherModals(except=null){[els.textOutputModal,els.assetPromptModal,els.mediaLightbox,els.previewFullscreenModal,els.providerModal,els.helpModal].forEach(modal=>{if(modal!==except)closeModalElement(modal);});}
function preparePrimarySurface(){closeControlDropdowns();hideMenus();collapseExpandedNodeComposers();}
...
function closeTopLayerOnEscape(){
  ...
  if(S.interaction){S.interaction.el?.remove?.();S.interaction=null;clearConnectionTargets();els.canvas.classList.remove('panning');return true;}
  if(!els.agentDrawer?.classList.contains('hidden-drawer')){hideDrawer('agentDrawer');return true;}
...
function setTimelineOpen(open){if(open)preparePrimarySurface();els.timelineShell?.classList.toggle('collapsed',!open);}
...
function renderAgentModels(){const selected=S.agentSession?.selectedProviderId&&`${S.agentSession.selectedProviderId}::${S.agentSession.selectedModelId}`,value=S.agentModels.some(m=>agentModelKey(m)===selected)?selected:agentModelKey(S.agentModels[0]||{});els.agentModelSelect.innerHTML=appSelectMarkup({value,options:S.agentModels.map(m=>({value:agentModelKey(m),label:m.modelId})),className:'agent-model-picker',ariaLabel:'策划模型'});$$('[data-app-select-option]',els.agentModelSelect).forEach(option=>option.addEventListener('click',e=>{e.stopPropagation();const menu=option.closest('.app-select');if(!menu||option.disabled)return;menu.dataset.value=option.dataset.appSelectValue;menu.querySelector('.app-select-value').textContent=option.textContent.trim();$$('[data-app-select-option]',menu).forEach(item=>{const active=item===option;item.classList.toggle('active',active);item.setAttribute('aria-selected',String(active));});closeDetailsMenu(menu);}));els.agentSendBtn.disabled=!S.agentModels.length;}
...
function setupGlobalInteractions(){
  ...
  els.canvas.addEventListener('contextmenu',e=>{const node=e.target.closest('.node'),edge=e.target.closest('.edge-hit');e.preventDefault();if(node){selectNode(node.dataset.id);showCanvasContextMenu(e.clientX,e.clientY,{nodeId:node.dataset.id});return;}if(edge){selectEdge(edge.dataset.edge);showCanvasContextMenu(e.clientX,e.clientY,{edgeId:edge.dataset.edge});return;}selectNode(null);showCanvasContextMenu(e.clientX,e.clientY,{world:screenToWorld(e.clientX,e.clientY)});});
...
els.newProjectBtn.addEventListener('click',async()=>{preparePrimarySurface();const name=prompt('项目名称','New Project');if(!name)return;const p=await api('/api/projects',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name})});S.projects.unshift(p);renderProjectSelect();await openProject(p.id);});
...
els.uploadBtn.addEventListener('click',()=>{preparePrimarySurface();S.uploadTargetNodeId=null;els.fileInput.click();});els.fileInput.addEventListener('change',()=>{const targetNodeId=S.uploadTargetNodeId;S.uploadTargetNodeId=null;uploadFiles([...els.fileInput.files],{targetNodeId});els.fileInput.value='';});els.fileInput.addEventListener('cancel',()=>{S.uploadTargetNodeId=null;});els.refreshAssetsBtn.addEventListener('click',refreshAssets);els.exportBtn.addEventListener('click',()=>{preparePrimarySurface();exportTimeline();});
