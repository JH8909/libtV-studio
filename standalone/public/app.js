...
function parseStructuredScript(text,preset){
  ...
  if(parsed){if(expected&&(!parsed.headers.every((header,index)=>header===expected[index])||parsed.headers.length!==expected.length))return null;if(preset==='video_script')parsed.rows=parsed.rows.map((row,index)=>row.length===expected.length-1&&/^\d+(?:\.\d+)?(?:\s*(?:秒|s))?$/i.test(String(row[3]||'').trim())?[String(index+1),...row]:row);return parsed;}
...
async function generateNode(nodeId){
  ...
  n.data.jobId='';n.data.status='queued';n.data.progress=0;n.data.error='';n.data.variantAssetIds=[];n.data.selectedVariantIndex=undefined;n.data.sequenceReviewStatus='';renderNode(n);
  try{
...
function agentProposalHtml(proposal){if(!proposal)return'';const p=proposal.plan,b=p.brief,s=p.styleBible,status=proposal.status==='pending'?'待应用':proposal.status==='applied'?'已应用':'已取代',realImage=S.models.some(m=>m.capabilities?.includes('image.generate')),realVideo=S.models.some(m=>m.capabilities?.includes('video.image_to_video')),warning=realImage&&realVideo?'':`<div class="agent-warning">${!realImage?'未配置真实图片模型。 ':''}${!realVideo?'未配置真实首帧视频模型。 ':''}节点仍会创建，可稍后配置。</div>`;return `<article class="agent-plan-card"><div class="agent-plan-head"><div><span class="agent-plan-status ${esc(proposal.status)}">${status}</span><h3>${esc(b.title)}</h3></div><span>${p.shots.length} 镜头 · ${b.targetDurationSec}s · ${esc(b.aspectRatio)}</span></div><p>${esc(b.logline)}</p><dl><div><dt>受众 / 平台</dt><dd>${esc(b.audience)} · ${esc(b.platform)}</dd></div><div><dt>基调 / 结尾</dt><dd>${esc(b.tone)} · ${esc(b.ending)}</dd></div><div><dt>视觉</dt><dd>${esc(s.visualStyle)} · ${esc(s.palette)}</dd></div><div><dt>镜头 / 光线</dt><dd>${esc(s.cameraLanguage)} · ${esc(s.lighting)}</dd></div></dl><div class="agent-story-arc">${p.storyArc.map((beat,i)=>`<span>${i+1}. ${esc(beat)}</span>`).join('')}</div><div class="agent-shots">${p.shots.map((shot,i)=>`<details><summary><strong>S${i+1} · ${esc(shot.title)}</strong><span>${shot.durationSec}s</span></summary><div><b>目的</b>${esc(shot.purpose)}<b>画面</b>${esc(shot.composition)}；${esc(shot.action)}<b>运镜 / 光线</b>${esc(shot.camera)}；${esc(shot.lighting)}<b>图片提示词</b>${esc(shot.imagePrompt)}<b>视频提示词</b>${esc(shot.videoPrompt)}<b>声音 / 连续性</b>${esc(shot.audioNote)}；${esc(shot.continuityNote)}</div></details>`).join('')}</div>${warning}<div class="agent-impact">先创建第 1 组图片和视频节点；验收当前视频后才会解锁下一段。</div>${proposal.status==='pending'?`<button class="primary agent-apply" data-agent-apply="${esc(proposal.id)}">应用为连续项目</button>`:''}</article>`;}
...
async function reviewSequenceClip(n,decision){const buttons=$$('.sequence-review-toolbar button',els.canvasWorld.querySelector(`[data-id="${n.id}"]`));buttons.forEach(button=>button.disabled=true);try{const result=await api(`/api/projects/${S.projectId}/sequence/clips/${n.data.sequenceClipId}/review`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({decision})});beginCanvasSnapshot();S.workflow=result.workflow;S.selectedNodeId=result.appliedNodeIds?.[0]||n.id;S.selectedNodeIds=S.selectedNodeId?[S.selectedNodeId]:[];S.selectedEdgeId=null;renderAll();if(result.appliedNodeIds?.length)requestAnimationFrame(()=>fitNodes(S.workflow.nodes.filter(node=>result.appliedNodeIds.includes(node.id))));toast(decision==='accept'?(result.nextClipId?'已接受本段，下一段已创建':'已接受，序列已完成'):'已拒绝本段，可调整后重新生成');}catch(e){buttons.forEach(button=>button.disabled=false);toast(friendlyError(e.message),'error');}}
async function applyAgentProposalUI(proposalId){const button=els.agentProposal.querySelector('[data-agent-apply]');if(button){button.disabled=true;button.textContent='应用中…';}try{const result=await api(`/api/projects/${S.projectId}/agent/proposals/${proposalId}/apply`,{method:'POST'});beginCanvasSnapshot();S.workflow=result.workflow;S.selectedNodeId=result.briefNodeId||result.appliedNodeIds?.[0]||null;S.selectedNodeIds=S.selectedNodeId?[S.selectedNodeId]:[];S.selectedEdgeId=null;const local=S.agentSession?.proposals?.find(p=>p.id===proposalId);if(local)Object.assign(local,result.proposal);renderAll();renderAgent();requestAnimationFrame(()=>fitNodes(S.workflow.nodes.filter(n=>result.appliedNodeIds.includes(n.id))));toast('连续项目已创建：请先完成并验收第 1 段');}catch(e){toast(e.message,'error');if(button){button.disabled=false;button.textContent='应用整份节点图';}}}
