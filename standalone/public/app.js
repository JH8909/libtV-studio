...
const S = {
  ...
  agent: { conversations: [], conversationId: '', messages: [], modelKey: localStorage.getItem('libtv.agentModel') || '', skillId: '', attachments: [], busy: false, error: '', modelMenuOpen: false, sourceMenuOpen: false, dictating: false, uploadFromPrompt: false, favoritesView: false },
};
...
function videoEmptyPlaceholder(){return `<div class="generator-empty generator-video-empty"><div class="generator-video-empty-play" aria-hidden="true">${icon('player-play')}</div></div>`;}
function videoGenerationPreview(n){if(nodeGenerationBusy(n))return generationPreviewBusy(n,'video');const out=generationPreviewAsset(n,'video');return out?`<div class="node-current-preview">${customVideoPlayerMarkup(out.publicUrl,{label:'当前生成视频'})}</div>`:videoEmptyPlaceholder();}
...
function setAgentStatus(message=''){if(els.agentStatus)els.agentStatus.textContent=String(message||'');}
...
async function deleteAgentConversation(conversationId){
  ...
  }catch(error){setAgentStatus(`删除失败：${error.message}`);}
}
...
async function sendAgentMessage(value){
  ...
  if(S.agent.skillId){
    ...
    if(!confirm(`将向当前画布添加「${skill?.name||'Skill'}」相关节点${autoRun?'，并自动开始运行':''}。\n\n对话本身不会改画布；只有确认使用技能才会改。继续？`))return;
  }
  const attachments=clone(S.agent.attachments||[]),productAssetId=attachments.find(asset=>asset.kind==='image')?.id||'';
  S.agent.busy=true;S.agent.favoritesView=false;S.agent.error='';S.agent.sourceMenuOpen=false;S.agent.modelMenuOpen=false;renderAgentPromptMenu();syncStudioChrome();setAgentStatus(S.agent.skillId?'正在应用技能到画布…':'正在整理创意…');await ensureAgentConversation();
  ...
  try{
    ...
    const reader=response.body?.getReader();if(!reader)throw new Error('Agent 没有返回可读取的响应');const decoder=new TextDecoder(),state={buffer:''};let doneMessage=null,doneWorkflow=null,doneSkillRun=null,streamFailed=false;const handle=(event,data)=>{
      if(event==='thinking')setAgentStatus(data.message||'正在整理创意…');
      ...
      if(event==='error'){streamFailed=true;pending.thinking=false;pending.text=`这次没有完成：${data.error||'未知错误'}`;pending.cards=[];pending.error=data.error||'';S.agent.error=data.error||'';setAgentStatus(data.retryable===false?'请求失败。':'请求失败，可以重新发送。');renderAgentMessages();}
  ...
  }catch(error){pending.thinking=false;pending.text=`这次没有完成：${error.message}`;pending.cards=[];renderAgentMessages();setAgentStatus('请求失败，可以重新发送。');}finally{S.agent.busy=false;syncStudioChrome();if(!S.agent.error)setAgentStatus('');}
}
...
function bindAgentWindow(){
  ...
  els.agentHistoryMenu?.addEventListener('click',async event=>{
    ...
    if(!button)return;
    try{await loadAgentConversation(button.dataset.agentConversation);els.agentHistoryMenu.classList.add('hidden');}catch(error){setAgentStatus(`打开会话失败：${error.message}`);}
  });
